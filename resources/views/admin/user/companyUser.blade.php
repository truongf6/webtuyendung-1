...
@section('contents')
    ...
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div>
            <form class="form-search" method="GET" action="{{ route('companyUser') }}">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-3 col-sm-6 col-12 mb-3">
                            <div class="text-nowrap">
                                ...
                                <a href="{{ route('companyUser') }}" class="btn btn-secondary rounded-pill ms-2"><i class="fas fa-times me-2"></i>Xóa lọc</a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <table class="table table-hover">
                    ...
                    @foreach($users as $user)
                        <tr data-id="{{$user->id}}">
                            ...
                            <td class="">
                                <button type="button" data-url="/company/users/{{$user->id}}" data-id="{{$user->id}}" class="btn btn-danger btnDeleteAsk px-2 me-2 py-1 fw-bolder" data-bs-toggle="modal" data-bs-target="#deleteModal{{$user->id}}">Xóa</button>
                ...
                <div class="modal fade ModelEditUser" id="editUser" tabindex="-1" aria-labelledby="editUserLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="card-body">
                                <form method='post' action='' enctype="multipart/form-data" class="editUserForm form-edit" id="form_usercompany_update">
                                    @method('PATCH')
                                    @csrf
                                    <div class='mb-3'>
                                        <label
                                    ...
                                    <input type="text" class="d-none" name="role_id" id="role-edit" value="2">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.btnEditUser').on('click', function() {
                ...
                $.ajax({
                    url: '/company/users/' + userID, // URL API để lấy thông tin người dùng
                    type: 'GET',
                    success: function(response) {
                        ...
                        $('#form_usercompany_update').attr('action', '/company/users/' + userID); // Cập nhật URL của form để sử dụng cho việc cập nhật
